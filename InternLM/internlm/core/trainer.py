...
class TrainState_two_dataset:
    ...
    def __init__(self, config, batch_sampler, wt_batch_sampler) -> None:
        """
        Args:
            config (Config): internlm config
            batch_sampler (torch.utils.data.Sampler): Because the dataloader loading is
        ...
        # The number of batches produced by the data iterator
        self.batch_count: int = 0
        self.img_motion_batch_count: int = 0
        self.wt_batch_count: int = 0
        # Used to store the number of samples consumed in the current epoch
        self.num_consumed_samples_in_epoch: int = 0
        # Total number of tokens consumed
        self.num_consumed_tokens: int = 0
        ...
        self.wt_num_consumed_samples_in_epoch: int = 0
        self.wt_num_consumed_tokens: int = 0
        self.wt_inf_nan_skip_batches: int = 0
        ...
        if wt_batch_sampler:
            self.init_wt_batch_sampler(wt_batch_sampler)

    def init_wt_batch_sampler(self, wt_batch_sampler):
        """
        Args:
            batch_sampler (torch.utils.data.Sampler): sampler.
        """
        # make a copy of batch_sampler.
        self.wt_batch_sampler = wt_batch_sampler.copy()
        # Iterator for the batch sampler
        self.wt_batch_sampler_iter = iter(self.wt_batch_sampler)
    ...
    def __str__(self) -> str:
        ...
        info = {
            "batch_count": self.batch_count,
            "img_motion_batch_count": self.img_motion_batch_count,
            "wt_batch_count": self.wt_batch_count,
            "num_consumed_samples_in_epoch": self.num_consumed_samples_in_epoch,
            "num_consumed_tokens": self.num_consumed_tokens,
            "inf_nan_skip_batches": self.inf_nan_skip_batches,
            "wt_num_consumed_samples_in_epoch": self.wt_num_consumed_samples_in_epoch,
            "wt_num_consumed_tokens": self.wt_num_consumed_tokens,
            "wt_inf_nan_skip_batches": self.wt_inf_nan_skip_batches,
            "step_count": self.step_count,
        }
    ...
    def load_state_dict(self, other_stuffs):
        ...
        self.num_consumed_tokens = other_stuffs["num_consumed_tokens"]
        self.inf_nan_skip_batches = other_stuffs["inf_nan_skip_batches"]
        self.wt_num_consumed_samples_in_epoch = other_stuffs["wt_num_consumed_samples_in_epoch"]
        self.wt_num_consumed_tokens = other_stuffs["wt_num_consumed_tokens"]
        self.wt_inf_nan_skip_batches = other_stuffs["wt_inf_nan_skip_batches"]

        # Because the ckpt save occurs after updating 'step_count',
        # there is no need to increment 'step_count' here (Does our step count start from 0 ?),
        # However, 'batch_count' is updating before ckpt storage, so it need to inc 1 when resume.
        self.batch_count = other_stuffs["batch_count"] + 1  # here you need to shift a batch backward
        self.wt_batch_count = other_stuffs["wt_batch_count"] + 1  # here you need to shift a batch backward
        self.img_motion_batch_count = other_stuffs.get("img_motion_batch_count", self.batch_count)
        self.step_count = other_stuffs.get("step_count", self.batch_count)
...
class Trainer:
    ...
    def step(self, spike=False):
        """Executes the parameter update step."""
        return self._engine.step(spike=spike)
