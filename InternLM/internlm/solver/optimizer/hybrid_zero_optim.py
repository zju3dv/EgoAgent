...
def split_params_into_different_groups_for_optimizer(
    ...
    for pgroup in param_groups:
        ...
        # assign param
        origin_params = []
        for idx, param in enumerate(pgroup["params"]):

            if param.dtype == torch.float32 and gpc.config.model.dtype != torch.float32:
                new_groups["fp32"]["params"].append(param)
                fp32_idx_list.append(idx)
            else:
                origin_params.append(param)
                fp16_idx_list.append(idx)
    ...
    return tuple(param_groups), fp16_idx_list, fp32_idx_list
...
class HybridZeroOptimizer(BaseOptimizer):
    ...
    def __init__(
        ...
        # new_param_groups, self.fp16_idx_list, self.fp32_idx_list\
        #     = split_params_into_different_groups_for_optimizer(optimizer.param_groups)
        ...
        # param_groups_global_index_list = [self.fp16_idx_list, self.fp32_idx_list]
        # # self.sorted_global_idx_mapingdict_list = []
        # total_params = []
        # for group_id, param_group in enumerate(self.optim.param_groups):
        #     total_params.extend(param_group["params"])
        # total_global_idx = []
        # for group_id, param_group in enumerate(self.optim.param_groups):
        #     total_global_idx.extend(param_groups_global_index_list[group_id])
        #
        # def get_sorted_global_idx_list(total_params, total_global_idx):
        #     sorted_params_with_idx = sorted(list(zip(total_params, total_global_idx)), key=lambda x: x[0].numel(), reverse=True)
        #     sorted_global_idx_list = [idx for _, idx in sorted_params_with_idx]
        #     return sorted_global_idx_list
        #
        # sorted_global_idx_list = get_sorted_global_idx_list(total_params, total_global_idx)
        # self.mapping_global_idx_to_sorted_global_idx = {global_idx: idx for idx, global_idx in enumerate(sorted_global_idx_list)}
        ...
        for group_id, param_group in enumerate(self.optim.param_groups):
            ...
            params_per_rank, no_params_ranks, _ \
                = self._partition_param_list(group_params)
            ...
            for rank, params in enumerate(params_per_rank):
                ...
                if len(params) != 0:
                    ### ignore the params that do not have gradients
    ...
    def _partition_param_list(self, param_list, global_idx_list=[]):
        no_params_ranks = []
        ...
        sorted_global_idx = []
        # sorted the param and record the changed list of global id
        if len(global_idx_list) != 0:
            param_list_with_idx = list(zip(param_list, global_idx_list))
            sorted_params_with_idx = sorted(param_list_with_idx, key=lambda x: x[0].numel(), reverse=True)
            sorted_params = [param for param, _ in sorted_params_with_idx]
            sorted_global_idx = [idx for _, idx in sorted_params_with_idx]
        else:
        ...
        return params_per_rank, set(no_params_ranks), sorted_global_idx
    ...
    def step(self, closure=None, spike=False):
        ...
        # grad = [param.grad for param in self._fp16_param_groups[0] if param.grad is not None]
        # nan_idx = [idx for idx, g in enumerate(grad) if torch.isnan(g).any()]
        ...
        return self._step(closure=closure, norms=total_norms, loss_spike=spike)

    def _step(self, closure=None, norms=None, loss_spike=False):
        assert closure is None, "closure is not supported by step()"
        ...
        if loss_spike:
            if gpc.is_rank_for_log():
                logger.warning("Loss spike occurs, please check it.")
                send_alert_message(
                    address=gpc.config.monitor.alert.feishu_alert_address,
                    message="Loss spike occurs, please check it.",
                )
            self._grad_store._averaged_gradients = dict()
            self.zero_grad()
            return False, norms
    ...
    def load_state_dict(self, states):
        ...
        if not set(states["flat_fp32_weights"].keys()) == set(self._fp32_flat_param_groups_of_current_rank):
            load_from_all_fp16_ckpt = True
        else:
            load_from_all_fp16_ckpt = False

        try:
            self.optim.load_state_dict(optim_states)
            flat_fp32_weights = states["flat_fp32_weights"]
        except:
            print('!Start to manually load all fp16 optim state_dict to fp16 and fp32 optim')
            with torch.no_grad():
                param_groups = optim_states["param_groups"]
                new_optim_states = {}
                param0 = param_groups[0]
                param0['optimizer_mode'] = ParallelMode.ZERO1
                param1={'name':'fp32'}
                param1.update(param0)
                param1['params']=[1]
                new_param_groups = [param0, param1]
                new_optim_states["param_groups"] = new_param_groups
                state = optim_states['state']

                state0 = state[0]
                state1={}
                state1['step']=state0['step']
                param_list = states["zero_devide_optim_plan"][0][0]

                param_idx_dict = {}
                offset = 0
                for global_idx in param_list:
                    if global_idx.endswith('_65536_1'):
                        continue
                    idx, *shape = global_idx.split('_')
                    numel = 1
                    for dim in shape:
                        numel *= int(dim)
                    int_idx = int(idx)
                    param_idx_dict[int_idx] = (offset, offset + numel)
                    offset += numel

                flat_fp32_weights = states["flat_fp32_weights"]

                new_flat_fp32_weights = {}

                fp16_flat_fp32_weights_list = []
                fp32_flat_fp32_weights_list = []

                fp16_exp_avg_list = []
                fp16_exp_avg_sq_list = []
                fp32_exp_avg_list = []
                fp32_exp_avg_sq_list = []
                total_exp_avg = state[0]['exp_avg']
                total_exp_avg_sq = state[0]['exp_avg_sq']

                for global_idx in self.fp16_idx_list + self.fp32_idx_list:
                    sorted_global_idx = self.mapping_global_idx_to_sorted_global_idx[global_idx]

                    if global_idx in self.fp16_idx_list:
                        try:
                            start = param_idx_dict[sorted_global_idx][0]
                            end = param_idx_dict[sorted_global_idx][1]
                        except:
                            print(f'sorted global idx {sorted_global_idx} has not grads')
                            continue
                        fp16_exp_avg_list.append(total_exp_avg[start:end])
                        fp16_exp_avg_sq_list.append(total_exp_avg_sq[start:end])
                        fp16_flat_fp32_weights_list.append(flat_fp32_weights[0][start:end])
                    elif global_idx in self.fp32_idx_list:
                        start = param_idx_dict[sorted_global_idx][0]
                        end = param_idx_dict[sorted_global_idx][1]
                        fp32_exp_avg_list.append(total_exp_avg[start:end])
                        fp32_exp_avg_sq_list.append(total_exp_avg_sq[start:end])
                        fp32_flat_fp32_weights_list.append(flat_fp32_weights[0][start:end])

                state0['exp_avg'] = torch.cat(fp16_exp_avg_list, dim=0)
                state0['exp_avg_sq'] = torch.cat(fp16_exp_avg_sq_list, dim=0)
                state1['exp_avg'] = torch.cat(fp32_exp_avg_list, dim=0)
                state1['exp_avg_sq'] = torch.cat(fp32_exp_avg_sq_list, dim=0)

                new_flat_fp32_weights[0] = torch.cat(fp16_flat_fp32_weights_list, dim=0)
                new_flat_fp32_weights[1] = torch.cat(fp32_flat_fp32_weights_list, dim=0)


            new_state = {0: state0, 1:state1}
            new_optim_states['state'] = new_state
            self.optim.load_state_dict(new_optim_states)
            # import pdb;pdb.set_trace()
            flat_fp32_weights = new_flat_fp32_weights
        ...
        if "zero_devide_optim_plan" in states and not load_from_all_fp16_ckpt:
