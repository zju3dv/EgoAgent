...
from internlm.core.engine import Engine, KDEngine, Engine_motion
...
def initialize_kd_trainer(
    ...
    if gpc.is_using_pp():
        ...
        if 'img_and_pose' in gpc.config.loss.name:
            scheduler = KDNonPipelineSchedulerContrastive_GPT(
                data_process_func=data_fn,
                gradient_accumulation_size=gpc.config.data.gradient_accumulation,
                scheduler_hooks=scheduler_hooks,
                momentum_scheduler=momentum_scheduler,
            )
        elif 'img_and_motion' in gpc.config.loss.name:
            scheduler = KDNonPipelineSchedulerContrastive_motion_regression(
                data_process_func=data_fn,
                gradient_accumulation_size=gpc.config.data.gradient_accumulation,
                scheduler_hooks=scheduler_hooks,
                momentum_scheduler=momentum_scheduler,
            )
        elif 'img_rep_motion' in gpc.config.loss.name:
            scheduler = KDNonPipelineSchedulerContrastive_motion_regression_dino(
                data_process_func=data_fn,
                gradient_accumulation_size=gpc.config.data.gradient_accumulation,
                scheduler_hooks=scheduler_hooks,
                momentum_scheduler=momentum_scheduler,
        ...
        elif gpc.config.loss.name.startswith('dino'):
            scheduler = KDNonPipelineSchedulerContrastive(
                data_process_func=data_fn,
                gradient_accumulation_size=gpc.config.data.gradient_accumulation,
                scheduler_hooks=scheduler_hooks,
                momentum_scheduler=momentum_scheduler,
...
def initialize_trainer_motion(
    model: nn.Module,
    optimizer: Optimizer,
    criterion: Optional[_Loss] = None,
    motion_criterion: Optional[_Loss] = None,
    train_dataloader: Optional[Iterable] = None,
    test_dataloader: Optional[Iterable] = None,
    lr_scheduler: Optional[_LRScheduler] = None,
    beta2_scheduler: Optional[Beta2Scheduler] = None,
    ...
    if gpc.is_using_pp():
        ...
        elif "latent_motion" in gpc.config.loss.name:
            scheduler = NonPipelineScheduler_motion_regression(
                data_process_func=data_fn,
                gradient_accumulation_size=gpc.config.data.gradient_accumulation,
                scheduler_hooks=scheduler_hooks,
                momentum_scheduler=momentum_scheduler,
